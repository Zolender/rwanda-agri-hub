                            ...
                            {['Date', 'Product ID', 'Category', 'Type', 'Quantity', 'Stock After', 'Region'].map((header, i) => (
                                ...
                                >
                                    <div className="flex items-center gap-2">
                                        {header}
                                        {i < 2 && <ArrowUpDown className={`w-3 h-3 ${isDark ? 'text-stone-500' : 'text-stone-400'}`} />}
                                    </div>
                                </th>
                            ))}
                        </tr>
                        ...
                        {transactions.length === 0 ? (
                            <tr>
                                <td colSpan={7} className="px-6 py-16 text-center">
                                    ...
                                    >
                                        ...
                                        <div>
                                            <p className={`text-lg font-semibold mb-1 ${isDark ? 'text-stone-300' : 'text-stone-900'}`}>No transactions found</p>
                                            <p className={`text-sm ${isDark ? 'text-stone-400' : 'text-stone-500'}`}>Try adjusting your filters or import some data</p>
                                        </div>
                                    </motion.div>
                                </td>
                            </tr>
                            ...
                            transactions.map((txn, index) => (
                                ...
                                >
                                    ...
                                    <td className={`px-6 py-4 whitespace-nowrap text-sm ${isDark ? 'text-stone-300' : 'text-stone-600'}`}>
                                    ...
                                    <td className={`px-6 py-4 whitespace-nowrap text-sm font-semibold ${isDark ? 'text-stone-200' : 'text-stone-900'}`}>
                                        {txn.quantityOrderedUnits.toLocaleString()} <span className={`font-normal ${isDark ? 'text-stone-400' : 'text-stone-500'}`}>{txn.product.unitOfMeasure}</span>
                                    </td>
                                    <td className={`px-6 py-4 whitespace-nowrap text-sm font-semibold ${isDark ? 'text-stone-200' : 'text-stone-900'}`}>
            ...
            {totalPages > 1 && (
                <div className={`px-6 py-4 border-t flex flex-col sm:flex-row items-center justify-between gap-4 ${
                    ...
                    <div className={`text-sm ${isDark ? 'text-stone-400' : 'text-stone-600'}`}>
                        Showing <span className={`font-semibold ${isDark ? 'text-stone-200' : 'text-stone-900'}`}>{(page - 1) * 50 + 1}</span> to{' '}
                        <span className={`font-semibold ${isDark ? 'text-stone-200' : 'text-stone-900'}`}>{Math.min(page * 50, totalCount)}</span> of{' '}
                        <span className={`font-semibold ${isDark ? 'text-stone-200' : 'text-stone-900'}`}>{totalCount.toLocaleString()}</span> transactions
                    </div>
